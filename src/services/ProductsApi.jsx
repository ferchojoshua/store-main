...
if (process.env.NODE_ENV === "production") {
  controller = `${REACT_APP_PRODURL}Productos/`;
} else {
...
export const getKardexAsync = async (token, data) => {
  const result = { statusResponse: true, data: [], error: null };
  let service = `${controller}GetKardex`;
  const authAxios = axios.create({
    baseURL: service,
    headers: {
      Authorization: `Bearer ${token}`,
...
export const getAllStoresKardexAsync = async (token, data) => {
  const result = { statusResponse: true, data: [], error: null };
  let service = `${controller}GetAllStoresKardex`;
  const authAxios = axios.create({
    baseURL: service,
    headers: {
      Authorization: `Bearer ${token}`,
...
export const getProductsListMAsync = async (token, data) => {
  const result = { statusResponse: true, data: [], error: null };
  let service = `${controller}getProductsListM`;
    const authAxios = axios.create({
      baseURL: service,
     headers: {
       Authorization: `Bearer ${token}`,
